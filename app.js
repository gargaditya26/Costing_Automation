...
function initMainAdmin(){
  ...
  document.querySelector('#saveWaMaster')?.addEventListener('click',()=>{localStorage.setItem('opsnoraWaMaster',document.querySelector('#waMasterMessage')?.value||'');localStorage.setItem('opsnoraWaNumber',document.querySelector('#waPhoneNumber')?.value||'');alert('WhatsApp settings saved')});
...
const OPSNORA_WA_TEMPLATE=`CARE ELECTRICALS
DATE : {{today}}

➖➖➖➖➖➖➖➖➖
♻️COPPER ROD RATE TODAY♻️: 
➖➖➖➖➖➖➖➖➖
Product: Rate/-{{Selling Price}}

CARE
📱+91-8802118802`;
const OPSNORA_OLD_WA_TEMPLATES=[
'CARE ELECTRICALS\n\n➖➖➖➖➖➖➖➖➖\n♻️COPPER ROD RATE TODAY♻️: {{today}}\n➖➖➖➖➖➖➖➖➖\nProduct: Rate/- {{Selling Price}}\n\nCARE\n📱+91-8802118802'
];
function getWaTemplate(){const saved=localStorage.getItem('opsnoraWaMaster');return (!saved||OPSNORA_OLD_WA_TEMPLATES.includes(saved))?OPSNORA_WA_TEMPLATE:saved;}
function waHeader(){return getWaTemplate().replace('Product: Rate/-{{Selling Price}}','{{PRODUCT_LINES}}');}
function renderWaMaster(){const e=document.querySelector('#waMasterMessage'),n=document.querySelector('#waPhoneNumber');if(e)e.value=getWaTemplate();if(n)n.value=localStorage.getItem('opsnoraWaNumber')||'8802118802';}
...
function calculationText(){let size=$('#size')?.value||'',wires=$('#wires')?.value||0,profit=$('#profit')?.value||0,length=$('#selectedLengthOut')?.textContent||'100M',cost=$('#bundleCost')?.textContent||money(0),selling=$('#bundleSelling')?.textContent||money(0),unit=$('#unitResult')?.textContent||'';let tpl=getWaTemplate();const vals={'{{today}}':new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'}),'{{Size}}':size,'{{Wires}}':wires,'{{Length}}':length,'{{Cost Price}}':cost,'{{Selling Price}}':selling,'{{Profit}}':profit,'{{Unit Cost}}':unit};return Object.entries(vals).reduce((txt,[k,v])=>txt.split(k).join(v),tpl)}
...
function historyItemText(c){const tpl=getWaTemplate();const vals={'{{today}}':new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'}),'{{Size}}':c.size??'','{{Wires}}':c.wires??'','{{Length}}':c.length??c.bundleLength??'','{{Cost Price}}':fmt(c.costPrice??c.cost??c.costPerUnit??0),'{{Selling Price}}':fmt(c.sellingPrice??c.selling??c.sp100??c.sp200??0),'{{Profit}}':c.profit??0,'{{Unit Cost}}':fmt(c.costPerUnit??0)};return Object.entries(vals).reduce((x,[k,v])=>x.split(k).join(v),tpl)}
function whatsappHistorySelected(){const items=getHistorySelected();if(!items.length)return alert('Select at least 1 calculation from history.');const text=items.map(x=>historyItemText(x)).join('\n\n--------------------\n\n');const phone=(localStorage.getItem('opsnoraWaNumber')||'').replace(/\D/g,'');const u=JSON.parse(localStorage.getItem('opsnoraUsage')||'{}');u.whatsappShares=(Number(u.whatsappShares)||0)+1;u.sharedValue=(Number(u.sharedValue)||0)+items.reduce((a,x)=>a+Number(x.sellingPrice??x.selling??x.sp100??x.sp200??0),0);localStorage.setItem('opsnoraUsage',JSON.stringify(u));window.open(phone?`https://wa.me/${phone}?text=${encodeURIComponent(text)}`:`https://wa.me/?text=${encodeURIComponent(text)}`,'_blank')}
if($('#shareHistory'))$('#shareHistory').onclick=async()=>{const items=getHistorySelected();if(!items.length)return alert('Select at least 1 calculation from history.');const lines=items.map((x,i)=>`Product ${i+1}: Rate/-${fmt(x.sellingPrice??x.selling??x.sp100??x.sp200??0)}`).join('\n');
const text=waHeader().replace('{{PRODUCT_LINES}}',lines).replace('{{today}}',new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'}));try{if(navigator.share)await navigator.share({title:'CARE Electrical Costing History',text});else{await navigator.clipboard.writeText(text);alert('Selected calculations copied to clipboard.')}}catch(e){}};
...
(function(){
  ...
  function historyMessage(c){
    ...
    const vals={'{{today}}':new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'}),'{{Size}}':c.size??'','{{Wires}}':c.wires??'','{{Length}}':c.length??c.bundleLength??'','{{Cost Price}}':fmt(c.costPrice??c.cost??c.costPerUnit??c.cost100??0),'{{Selling Price}}':fmt(c.sellingPrice??c.selling??c.sp100??0),'{{Profit}}':c.profit??0,'{{Unit Cost}}':fmt(c.costPerUnit??0)};
  ...
  async function shareSelectedHistory(){
    ...
    const lines=items.map((item,i)=>`Product ${i+1}: Rate/-${fmt(item.sellingPrice??item.selling??item.sp100??item.sp200??0)}`).join('\n');
    const text=waHeader().replace('{{PRODUCT_LINES}}',lines).replace('{{today}}',new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'2-digit',year:'numeric'}));
    try{
