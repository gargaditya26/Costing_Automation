...
if(pathPage()!=='index'&&!sessionStorage.opsnoraCare){location.href='../index.html'}
...
function renderTasks(){if(!$('#taskList'))return;$('#taskList').innerHTML=data.tasks.map(t=>`<div class="task"><div class="task-top"><div><b>${t.title}</b><small>Assigned: ${t.assignee} · Due: ${t.due||'Not set'}</small></div><span class="status ${taskStatusClass(t.status)}">${t.status}</span></div><div class="task-actions"><select data-task-status="${t.id}"><option ${t.status==='Pending'?'selected':''}>Pending</option><option ${t.status==='Completed'?'selected':''}>Completed</option><option ${t.status==='Hold'?'selected':''}>Hold</option></select><button class="btn danger" data-delete-task="${t.id}">Delete</button></div></div>`).join('')||'<div class="empty">No assigned work yet. Click “Assign Work” to create a task.</div>'}
function renderProducts(){if(!$('#productsGrid'))return;$('#productsGrid').innerHTML=data.products.map((p,i)=>`<div class="card product"><div class="pico">◇</div><h3>${p.size} inch</h3><p class="muted">${p.wires||0} wires · Rule ${p.rule||0}</p><div class="lines"><div>Rod Rate <b>${money(p.rodRate)}</b></div><div>PVC Rate <b>${money(p.pvcRate)}</b></div><div>Weight <b>${p.weight||0} kg</b></div><div>Labour <b>${p.labour||0}%</b></div></div><button class="btn secondary" data-edit-product="${i}" style="margin-top:13px">Edit Master</button></div>`).join('')}
...
function bind(){if($('#menu'))$('#menu').onclick=()=>$('#side').classList.toggle('open');if($('#logoutBtn'))$('#logoutBtn').onclick=()=>{sessionStorage.removeItem('opsnoraCare');location.href='../index.html'};if($('#notifBtn'))$('#notifBtn').onclick=e=>{e.stopPropagation();notify()};if($('#themeToggle')){$('#themeToggle').onclick=()=>{document.body.classList.toggle('light');localStorage.opsTheme=document.body.classList.contains('light')?'light':'dark';$('#themeToggle').textContent=document.body.classList.contains('light')?'🌙 Dark':'☀ Light'}}if(localStorage.opsTheme==='light'){document.body.classList.add('light');if($('#themeToggle'))$('#themeToggle').textContent='🌙 Dark'}if($('#closeModal'))$('#closeModal').onclick=closeModal;if($('#modal'))$('#modal').onclick=e=>{if(e.target.id==='modal')closeModal()};if($('#size'))$('#size').onchange=loadMaster;if($('#loadMaster'))$('#loadMaster').onclick=loadMaster;if($('#calcBundle'))$('#calcBundle').onclick=()=>calculateBundle(true);if($('#calcUnit'))$('#calcUnit').onclick=calculateUnit;
...
function calculationText(){let size=$('#size')?.value||'', wires=$('#wires')?.value||0, profit=$('#profit')?.value||0, cost100=$('#bundle100')?.textContent||money(0),cost200=$('#bundle200')?.textContent||money(0),sp100=$('#sp100')?.textContent||money(0),sp200=$('#sp200')?.textContent||money(0);return `OPSNORA · CARE Electrical\nCosting & Pricing Calculation\n\nSize: ${size} inch\nWires: ${wires}\n\n100M Cost: ${cost100}\n100M Selling Price: ${sp100}\n\n200M Cost: ${cost200}\n200M Selling Price: ${sp200}\n\nProfit: ${profit}%\nGenerated: ${new Date().toLocaleString('en-IN')}`}
...
(function(){
  ...
  function init(){
    ...
    q('#historyType')?.addEventListener('change',renderHistory);
    restoreLast();
    updateLength();
    renderHistory();
  }
  ...
  function renderHistory(){
    const t=q('#calcTable'),head=q('#historyHead'),mode=q('#historyType')?.value||'bundle';
    if(!t||!head)return;
    if(mode==='unit'){
      head.innerHTML='<tr><th>Time</th><th>Bundle</th><th>Unit Length</th><th>Bundle Cost</th><th>Moulding</th><th>Profit %</th><th>Unit Cost</th><th>Selling Price</th></tr>';
      t.innerHTML=data.unitCalculations.map(c=>`<tr><td>${c.time||''}</td><td>${c.bundleLength||''} · ${c.size??''}</td><td>${c.length??''}</td><td>${fmt(c.bundleCost)}</td><td>${fmt(c.moulding)}</td><td>${c.profit??0}%</td><td>${fmt(c.costPerUnit)}</td><td>${fmt(c.sellingPrice)}</td></tr>`).join('')||'<tr><td colspan="8">No unit-wise calculations saved yet.</td></tr>';
    }else{
      head.innerHTML='<tr><th>Time</th><th>Length</th><th>Size</th><th>Wires</th><th>Cost Price</th><th>Profit %</th><th>Selling Price</th></tr>';
      t.innerHTML=data.calculations.map(c=>`<tr><td>${c.time||''}</td><td>${c.length||'100M'}</td><td>${c.size??''}</td><td>${c.wires??0}</td><td>${fmt(c.costPrice??c.cost100)}</td><td>${c.profit??0}%</td><td>${fmt(c.sellingPrice??c.sp100)}</td></tr>`).join('')||'<tr><td colspan="7">No bundle-wise calculations saved yet.</td></tr>';
    }
  }
