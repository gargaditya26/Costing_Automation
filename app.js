...
function licenseLockCard(feature){return `<div class="license-lock-card"><div class="license-lock-icon">🔒</div><h2>Not Available in Current Plan</h2><p class="muted">${OPSNORA_LICENSE_LABELS[feature]||'This feature'} is not available in the current plan.</p><small>Contact OPSNORA Team to enable access.</small></div>`}
function addPageLicenseOverlay(feature){const root=document.querySelector('#pageRoot');if(!root||root.querySelector('.license-page-overlay'))return;root.classList.add('license-page-locked');root.insertAdjacentHTML('beforeend',`<div class="license-page-overlay" aria-label="${OPSNORA_LICENSE_LABELS[feature]||'Feature'} unavailable">${licenseLockCard(feature)}</div>`)}
function addSubLicenseOverlay(el,feature){if(!el||el.querySelector('.license-sub-overlay'))return;el.classList.add('license-sublocked');el.insertAdjacentHTML('beforeend',`<div class="license-sub-overlay">${licenseLockCard(feature)}</div>`)}
function applyLicenseGuards(){const page=pathPage(),l=opsnoraLicense();if(page==='main-admin'||page==='index')return;const feature=page==='costing'?'costing_bundle':page;if(l[feature]===false)addPageLicenseOverlay(feature);if(page==='costing'){const unitTab=document.querySelector('[data-mode="unit"]'),unitFields=document.querySelector('#unitFields');if(l.costing_unit===false){if(unitTab){unitTab.classList.add('license-locked-tab');unitTab.onclick=(e)=>{e.preventDefault();addSubLicenseOverlay(unitFields,'costing_unit')}}addSubLicenseOverlay(unitFields,'costing_unit')}}}
...
function bind(){if($('#menu'))$('#menu').onclick=()=>$('#side').classList.toggle('open');if($('#logoutBtn'))$('#logoutBtn').onclick=()=>{sessionStorage.removeItem('opsnoraCare');location.href='../index.html'};if($('#notifBtn'))$('#notifBtn').onclick=e=>{e.stopPropagation();notify()};if($('#closeModal'))$('#closeModal').onclick=closeModal;if($('#modal'))$('#modal').onclick=e=>{if(e.target.id==='modal')closeModal()};if($('#size'))$('#size').onchange=loadMaster;if($('#loadMaster'))$('#loadMaster').onclick=loadMaster;if($('#calcBundle'))$('#calcBundle').onclick=()=>calculateBundle(true);if($('#calcUnit'))$('#calcUnit').onclick=calculateUnit;
